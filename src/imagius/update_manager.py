...
class UpdateManager(QtCore.QObject):
    ...
    def _request_finished(self, reply):
        if reply.error() == QtNetwork.QNetworkReply.NoError:
            ...
            if latest_release > Version(settings.app_version) and is_draft is False:
                ...
                self._msgbox.exec()
                if self._msgbox.clickedButton() == self._msgox_btn_ok:
                    QtGui.QDesktopServices.openUrl(QtCore.QUrl(asset_url.toString()))
            else:
            ...
            LOGGER.error("Update Manager: Unable to retrieve updates from release api (Error: %s)" % error_code)
            self._setup_msgbox("Error checking for Updates",
    ...
    def _setup_msgbox(self, title, text, details=None, btn_accept_title="Ok", btn_reject_title="Cancel"):
        ...
        self._msgox_btn_ok = self._msgbox.addButton(btn_accept_title, QtWidgets.QMessageBox.AcceptRole)
        self._msgbox_btn_cancel = self._msgbox.addButton(btn_reject_title, QtWidgets.QMessageBox.RejectRole)
